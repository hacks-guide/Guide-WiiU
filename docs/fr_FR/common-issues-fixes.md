...
- **FSGetMountSource failed:** Identique au -3 ci-dessus, signifie que la carte SD n'est pas détectée. Réinsérez la SD et réessayez. Cela pourrait également signifier que la carte SD est verrouillée (le loquet de verrouillage est en bas au lieu d'être en haut) ou que la carte SD est nommée "WIIU", ce qui est indiqué dans le guide que cela provoque des conflits. Si ce n'est pas un de ces problèmes, reportez-vous à la dernière suggestion de -3.
...
Cela est dû à une sous-alimentation en électricité du disque dur puisque la Wii U n'en fournit pas assez au travers d'un seul port USB.
