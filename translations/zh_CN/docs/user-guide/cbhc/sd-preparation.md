...
?> **注意** 你的 SD 卡需要被格式化为 FAT32。 如果你的 SD 卡的格式不是 FAT32，则你需要用 [GUIFormat](http://ridgecrop.co.uk/index.htm?guiformat.htm) 格式化，格式化时请将分配单元大小设置为 32k（32768）。 建议不要将 SD 卡卷标命名为 `wiiu` 或 `cbhc`，因为它可能会引起一些程序的问题。
